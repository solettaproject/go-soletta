...
sol_flow_simple_c_type_new_full(const char *name, size_t private_data_size, uint16_t options_size, int (*func)(struct sol_flow_node *node, const struct sol_flow_simple_c_type_event *ev, void *data), struct CPortDescription *ports, int port_count)
{
    ...
    {
        port_name = ports[idx].name;
        const struct sol_flow_packet_type *pt = ports[idx].packet_type;
        int direction = ports[idx].direction;
